...
for pos in mapa:
    for dr in range(-20, 21):
        for di in range(-20 + abs(dr), 21 - abs(dr)):
            assert abs(dr) + abs(di) <= 20
            move = dr * 1 + di * 1j
            cost = abs(dr) + abs(di)

            if pos + move in mapa:
                if costs[pos + move] + cost < costs[pos]:
                    cheatCount[costs[pos] - cost - costs[pos + move]] += 1
                    if costs[pos] - cost - costs[pos + move] >= 100:
                        counts += 1


print(costs[start_position])
print(sorted(cheatCount.items(), key=lambda a: a[0]))
print(counts)
